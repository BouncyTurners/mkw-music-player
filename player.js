...
function playTrack(index) {
    ...
    if (audioPlayer.readyState >= 1) {
        // Metadata al beschikbaar (cached track)
        metadataHandler();
    } else {
        audioPlayer.addEventListener('loadedmetadata', metadataHandler);
    }
...
function playPrevTrack() {
    ...
    if (playInOrder) {
        ...
        playNextTrack(); // Optional: shuffle history niet ondersteund
    }
}
...
function seek(e) {
    const duration = audioPlayer.duration;
    if (isNaN(duration) || duration === 0) return audioPlayer.currentTime;
